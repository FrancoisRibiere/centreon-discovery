...
* Le paquet maraidb-devel (v5.5 sur CentOS 6.7) doit être installé sur le central.
* Les paquets  zlib-devel et openssl-devel (sur CentOS) doient être installés sur le central.
...
Choississez votre type d'installation :
 ...
 - en tant que les deux (si installé sur le mêmeserveur que Centreon) : "-t both"
 
Exemple pour poller (bash permet d'éviter la déconnexion) : # bash ./install.sh -i -t poller
...
Si vous n'avez pas autorisé le démarrage automatique:
...
Le chemin "/usr/share/centreon/www/modules" est à adapter en fonction de votre configuration.
